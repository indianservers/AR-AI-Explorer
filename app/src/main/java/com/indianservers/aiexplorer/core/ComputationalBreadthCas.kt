...
internal object ComputationalBreadthCas {
    ...
    private fun primeFactors(value:BigInteger):Map<BigInteger,Int>{val two=BigInteger.valueOf(2);var n=value.abs();val out=linkedMapOf<BigInteger,Int>();var p=two;while(p*p<=n){while(n%p==BigInteger.ZERO){out[p]=(out[p]?:0)+1;n/=p};p=if(p==two)BigInteger.valueOf(3)else p+two};if(n>BigInteger.ONE)out[n]=(out[n]?:0)+1;return out}
