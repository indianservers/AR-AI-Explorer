...
import com.indianservers.aiexplorer.smartboard.canvas.SmartBoardCanvasView
import com.indianservers.aiexplorer.smartboard.canvas.SmartBoardStrokeStyle
...
import com.indianservers.aiexplorer.smartboard.models.BiologyResultElement
import com.indianservers.aiexplorer.smartboard.models.ChemistryExpressionElement
import com.indianservers.aiexplorer.smartboard.models.ChemistryResultElement
import com.indianservers.aiexplorer.smartboard.models.EnglishTextElement
import com.indianservers.aiexplorer.smartboard.models.EnglishResultElement
...
import com.indianservers.aiexplorer.smartboard.models.PhysicsResultElement
...
import com.indianservers.aiexplorer.smartboard.models.SolutionSequenceElement
...
import com.indianservers.aiexplorer.smartboard.tools.SmartBoardReconstructionKind
import com.indianservers.aiexplorer.smartboard.models.SmartBoardSubject
import com.indianservers.aiexplorer.smartboard.models.SmartBoardIntelligenceMode
...
import com.indianservers.aiexplorer.smartboard.recognition.OfflineFormulaIdentifier
...
import com.indianservers.aiexplorer.latexStyleFormula
import com.indianservers.aiexplorer.smartboard.tutor.SmartBoardTutorVerificationStatus
    ...
    BoxWithConstraints(
        ...
        if (settingsOpen) {
            OverlayPanel("Board Settings", onDismiss = { settingsOpen = false }) {
                ...
                Text("Board subject", color = BoardInk, fontWeight = FontWeight.Bold)
                listOf(
                    ...
                    SmartBoardSubject.GENERAL,
                ).forEach { subject ->
                    BoardButton(
                        if (vm.document.subjectMode.selection == subject) "✓ ${subject.displayName()}" else subject.displayName(),
                    ) { vm.setBoardSubject(subject) }
                }
                SettingSwitch("Lock Board subject", vm.document.subjectMode.locked, vm::setSubjectLock)
                if (vm.selectedIds.isNotEmpty()) {
                    Text("Classify selected content only", color = BoardInk, fontWeight = FontWeight.Bold)
                    listOf(
                        SmartBoardSubject.MATHEMATICS, SmartBoardSubject.PHYSICS, SmartBoardSubject.CHEMISTRY,
                        SmartBoardSubject.ENGLISH, SmartBoardSubject.BIOLOGY,
                    ).forEach { subject ->
                        BoardButton(subject.displayName()) { vm.assignSelectedSubject(subject) }
                    }
                }
    ...
    val moreMenu: @Composable () -> Unit = {
        Box {
            ...
            DropdownMenu(expanded = moreOpen, onDismissRequest = onDismissMore) {
                DropdownMenuItem(text = { Text("New Smart Board") }, onClick = { onNew(SmartBoardSubject.AUTO) })
    ...
    FlowRow(
        ...
        vm.selectedShape?.let { shape ->
            ...
            if (shape.shapeType in setOf(
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.COORDINATE_AXES,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.GRAPH_GRID,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.NUMBER_LINE,
                )
            ) {
                BoardButton("Open Graph 2D") { onHandoff("graph2d", "") }
            }
            if (shape.shapeType in setOf(
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.TRIANGLE,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.RIGHT_TRIANGLE,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.EQUILATERAL_TRIANGLE,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.CIRCLE,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.RECTANGLE,
                    com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.POLYGON,
                )
            ) {
        ...
        vm.selectedPhysicsActions.forEach { action ->
            BoardButton(action.label(), enabled = !vm.runningAction) { vm.runPhysicsAction(action, onHandoff) }
        }
        if (vm.selectedExpression != null) {
            ...
            if (vm.selectedExpression?.semanticTree?.root?.kind ==
                com.indianservers.aiexplorer.smartboard.models.SemanticMathNodeKind.MATRIX
            ) {
...
private fun RecognitionPanel(vm: SmartBoardViewModel, modifier: Modifier) {
    ...
    Column(
        ...
        review.subjectDetection?.let { detection ->
            Text(
                "Detected subject: ${detection.primarySubject?.displayName() ?: "Unresolved"} · ${detection.confidenceLevel.name.lowercase()} confidence",
                color = if (detection.requiresConfirmation) BoardWarning else BoardCyan,
                modifier = Modifier.semantics {
                    liveRegion = LiveRegionMode.Polite
                    contentDescription = "Detected subject ${detection.primarySubject?.displayName() ?: "unresolved"}. ${detection.confidenceLevel.name.lowercase()} confidence."
                },
            )
            detection.detectedConcepts.firstOrNull()?.let { Text("Concept: ${it.displayName}", color = BoardViolet) }
            Text(
                detection.candidates.firstOrNull()?.evidence?.take(2)?.joinToString(". ") { it.description }.orEmpty(),
                color = BoardMuted,
                fontSize = 10.sp,
            )
            Text("Use for this content", color = BoardInk, fontWeight = FontWeight.Bold)
            FlowRow(horizontalArrangement = Arrangement.spacedBy(6.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                detection.candidates.map { it.subject }
                    .plus(listOf(SmartBoardSubject.MATHEMATICS, SmartBoardSubject.PHYSICS, SmartBoardSubject.CHEMISTRY, SmartBoardSubject.ENGLISH, SmartBoardSubject.BIOLOGY))
                    .distinct().take(5).forEach { subject ->
                        BoardButton(if (review.selectedSubject == subject) "✓ ${subject.displayName()}" else subject.displayName()) {
                            vm.chooseRecognitionSubject(subject)
                        }
                    }
            }
        }
        ...
        FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            BoardButton(
                if (review.selectedSubject in setOf(SmartBoardSubject.MATHEMATICS, SmartBoardSubject.PHYSICS)) "Insert expression" else "Insert content",
    ...
    LazyColumn(Modifier.fillMaxWidth().heightIn(max = 480.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
        items(outlineElements, key = SmartBoardElement::id) { element ->
            ...
            Column(
                ...
                if (element is PhysicsExpressionElement) {
                    var edit by remember(element.id, element.displaySource) { mutableStateOf(element.displaySource) }
                    OutlinedTextField(edit, { edit = it }, Modifier.fillMaxWidth(), label = { Text("Edit Physics notation") })
                    FlowRow(horizontalArrangement = Arrangement.spacedBy(6.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        BoardButton("Apply Physics notation") { vm.editPhysicsExpression(element, edit) }
                    }
                    element.ambiguities.forEach { ambiguity -> Text(ambiguity, color = BoardWarning, fontSize = 11.sp) }
                }
                ...
                if (element is ImageElement) {
                    FlowRow(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        ...
                        if (vm.document.subject == SmartBoardSubject.PHYSICS) {
                            BoardButton("Recognize Physics") { vm.select(setOf(element.id)); vm.recognizeSelectedPhysicsImage() }
                        }
                    }
                }
            }
...
private fun com.indianservers.aiexplorer.smartboard.models.SmartBoardShapeType.shapeLabel() =
...
private fun com.indianservers.aiexplorer.smartboard.models.PhysicsActionType.label() =
    name.lowercase().replace('_', ' ').replaceFirstChar(Char::titlecase)
