package com.indianservers.aiexplorer.smartboard.media
...
object SmartBoardRegionEngine {
    ...
    fun move(regions: List<RecognitionRegion>, id: String, dx: Float, dy: Float) =
        regions.map { if (it.id == id) it.copy(bounds = it.bounds.translate(com.indianservers.aiexplorer.smartboard.models.SmartBoardPoint(dx, dy))) else it }.reindexed()
    ...
    fun merge(regions: List<RecognitionRegion>, ids: Set<String>): List<RecognitionRegion> {
        ...
        val mergedBounds = SmartBoardBounds.from(selected.flatMap {
            listOf(com.indianservers.aiexplorer.smartboard.models.SmartBoardPoint(it.bounds.left, it.bounds.top), com.indianservers.aiexplorer.smartboard.models.SmartBoardPoint(it.bounds.right, it.bounds.bottom))
        })
