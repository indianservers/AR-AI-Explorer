...
class LocalLearningAssistantProvider : LearningAssistantProvider {
    ...
    private val knowledgeIndex = LocalAssistantKnowledgeIndex()
    override suspend fun respond(request: GroundedAssistantRequest): AssistantResponse {
        val intent = OfflineAssistantIntentClassifier.classify(request.learnerQuestion)
        val searchResults = knowledgeIndex.search(request.learnerQuestion, conceptId = request.conceptId)
        val validation = GroundingValidator.validate(request)
        if (!validation.valid) return AssistantResponse("", AssistantResponseType.FALLBACK, emptyList(), emptyList(), AssistantVerificationStatus.REJECTED, validation.issues, id, detectedIntent = intent, localSearchResults = searchResults)
        ...
        val text = when {
            intent == AssistantIntent.DEFINITION -> definition.text
            intent == AssistantIntent.SEARCH && searchResults.isNotEmpty() -> "I found this reviewed local match: ${searchResults.first().title}. ${searchResults.first().snippet}"
            intent == AssistantIntent.FORMULA -> "Use only a reviewed formula or identity for this concept, then check units and conditions. ${hint?.text ?: definition.text}"
        ...
        return AssistantResponse(text, if (request.learnerSteps.isEmpty()) AssistantResponseType.SOCRATIC_QUESTION else AssistantResponseType.STEP_FEEDBACK, reference, visual, AssistantVerificationStatus.LOCALLY_AUTHORED, emptyList(), id, detectedIntent = intent, localSearchResults = searchResults)
    }
}
