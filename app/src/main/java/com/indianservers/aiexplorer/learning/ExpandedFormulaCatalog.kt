...
internal fun expandedFormulaCatalog(): List<ExpandedFormulaSeed> = buildList {
    ...
    addAll(essentialFormulaAdditions())
}

private fun essentialFormulaAdditions(): List<ExpandedFormulaSeed> = listOf(
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Percentage change", """\operatorname{Change}\%=\frac{\operatorname{new}-\operatorname{old}}{\operatorname{old}}\times100\%""", "new,old", "Compare increase or decrease against the original value."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Profit percentage", """\operatorname{Profit}\%=\frac{\operatorname{SP}-\operatorname{CP}}{\operatorname{CP}}\times100\%""", "SP,CP", "Find profit as a percentage of cost price."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Discounted price", """\operatorname{Price}=M\left(1-\frac{d}{100}\right)""", "M,d", "Find selling price after a discount rate."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "GST amount", """\operatorname{GST}=\frac{r}{100}P""", "r,P", "Calculate tax amount from taxable price and GST rate."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Simple interest", """I=\frac{PRT}{100}""", "I,P,R,T", "Calculate interest without compounding."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Compound amount annual", """A=P\left(1+\frac{r}{100}\right)^{n}""", "A,P,r,n", "Calculate amount after annual compounding."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Speed distance time", """d=s\,t""", "d,s,t", "Relate distance, speed, and time."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Functions and Transformations", "Average speed", """\bar{s}=\frac{\operatorname{total\ distance}}{\operatorname{total\ time}}""", "s", "Find overall speed across a journey."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Quadratics", "Quadratic formula", """x=\frac{-b\pm\sqrt{b^{2}-4ac}}{2a}""", "x,a,b,c", "Solve a quadratic equation ax squared plus bx plus c equals zero."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Quadratics", "Quadratic discriminant", """D=b^{2}-4ac""", "D,a,b,c", "Classify the roots of a quadratic equation."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Quadratics", "Quadratic sum of roots", """\alpha+\beta=-\frac{b}{a}""", "alpha,beta,a,b", "Relate quadratic roots to coefficients."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Quadratics", "Quadratic product of roots", """\alpha\beta=\frac{c}{a}""", "alpha,beta,a,c", "Relate the product of quadratic roots to coefficients."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Sequences", "Arithmetic progression nth term", """a_{n}=a+\left(n-1\right)d""", "a_n,a,n,d", "Find any term of an arithmetic progression."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Sequences", "Arithmetic progression sum", """S_{n}=\frac{n}{2}\left[2a+\left(n-1\right)d\right]""", "S,n,a,d", "Sum the first n terms of an arithmetic progression."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Sequences", "Geometric progression nth term", """a_{n}=ar^{n-1}""", "a_n,a,r,n", "Find any term of a geometric progression."),
    expandedFormula(FormulaCategory.AlgebraFunctions, "Sequences", "Geometric progression sum", """S_{n}=a\frac{1-r^{n}}{1-r},\quad r\ne1""", "S,n,a,r", "Sum the first n terms of a geometric progression."),
    expandedFormula(FormulaCategory.GeometryMensuration, "Plane Geometry", "Triangle area base height", """A=\frac{1}{2}bh""", "A,b,h", "Find triangle area from base and perpendicular height."),
    expandedFormula(FormulaCategory.GeometryMensuration, "Triangles", "Heron formula", """A=\sqrt{s\left(s-a\right)\left(s-b\right)\left(s-c\right)}""", "A,s,a,b,c", "Find triangle area from three side lengths."),
    expandedFormula(FormulaCategory.GeometryMensuration, "Triangles", "Triangle semiperimeter", """s=\frac{a+b+c}{2}""", "s,a,b,c", "Find the semiperimeter used in Heron's formula."),
    expandedFormula(FormulaCategory.GeometryMensuration, "Circles", "Arc length degrees", """s=\frac{\theta}{360^{\circ}}2\pi r""", "s,theta,r", "Find arc length when the central angle is in degrees."),
    expandedFormula(FormulaCategory.Trigonometry, "Trigonometric Identities", "Pythagorean identity sine cosine", """\sin^{2}\theta+\cos^{2}\theta=1""", "theta", "Simplify expressions containing sine and cosine."),
    expandedFormula(FormulaCategory.Trigonometry, "Trigonometric Identities", "Tangent Pythagorean identity", """1+\tan^{2}\theta=\sec^{2}\theta""", "theta", "Convert tangent and secant expressions."),
    expandedFormula(FormulaCategory.Trigonometry, "Radians", "Degree radian conversion", """\theta_{\mathrm{rad}}=\theta_{\deg}\frac{\pi}{180^{\circ}}""", "theta", "Convert angle measure from degrees to radians."),
    expandedFormula(FormulaCategory.CalculusAnalysis, "Integrals", "Fundamental theorem of calculus", """\int_{a}^{b}f\left(x\right)\,dx=F\left(b\right)-F\left(a\right)""", "f,F,a,b,x", "Evaluate a definite integral using an antiderivative.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.CalculusAnalysis, "Multivariable Calculus", "Lagrange multiplier condition", """\nabla f=\lambda\nabla g""", "f,g,lambda", "Optimize a function subject to an equality constraint.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Matrices", "Two by two inverse", """A^{-1}=\frac{1}{ad-bc}\begin{bmatrix}d&-b\\-c&a\end{bmatrix}""", "A,a,b,c,d", "Invert a two by two matrix when its determinant is nonzero.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Determinants", "Two by two determinant", """\det\begin{bmatrix}a&b\\c&d\end{bmatrix}=ad-bc""", "a,b,c,d", "Find the determinant of a two by two matrix.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Systems", "Cramer rule", """x_{i}=\frac{\det A_{i}}{\det A}""", "x,A,i", "Solve a square linear system using determinants.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Eigenvalues", "Characteristic equation", """\det\left(A-\lambda I\right)=0""", "A,lambda,I", "Find eigenvalues from the characteristic polynomial.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Projections", "Vector projection", """\operatorname{proj}_{\mathbf{b}}\mathbf{a}=\frac{\mathbf{a}\cdot\mathbf{b}}{\mathbf{b}\cdot\mathbf{b}}\mathbf{b}""", "a,b", "Project one vector onto another.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.LinearAlgebraVectors, "Decompositions", "QR decomposition", """\mathbf{A}=\mathbf{Q}\mathbf{R}""", "A,Q,R", "Factor a matrix into orthogonal and triangular parts.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.CoordinateGeometry3D, "Lines", "Slope formula", """m=\frac{y_{2}-y_{1}}{x_{2}-x_{1}}""", "m,x_1,x_2,y_1,y_2", "Find the slope of a line through two points."),
    expandedFormula(FormulaCategory.CoordinateGeometry3D, "Lines", "Midpoint formula", """M=\left(\frac{x_{1}+x_{2}}{2},\frac{y_{1}+y_{2}}{2}\right)""", "M,x_1,x_2,y_1,y_2", "Find the midpoint of a segment."),
    expandedFormula(FormulaCategory.CoordinateGeometry3D, "Lines", "Coordinate triangle area", """A=\frac{1}{2}\left|x_{1}\left(y_{2}-y_{3}\right)+x_{2}\left(y_{3}-y_{1}\right)+x_{3}\left(y_{1}-y_{2}\right)\right|""", "A,x_1,x_2,x_3,y_1,y_2,y_3", "Find triangle area from vertex coordinates."),
    expandedFormula(FormulaCategory.CoordinateGeometry3D, "Planes", "Plane equation normal form", """\mathbf{n}\cdot\left(\mathbf{r}-\mathbf{r}_{0}\right)=0""", "n,r,r_0", "Represent a plane using a normal vector and a point.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.CoordinateGeometry3D, "3D Distance", "Direction cosines", """l^{2}+m^{2}+n^{2}=1""", "l,m,n", "Check direction cosines of a line in 3D."),
    expandedFormula(FormulaCategory.ProbabilityCombinatorics, "Counting", "Permutation formula", """{}^{n}P_{r}=\frac{n!}{\left(n-r\right)!}""", "n,r", "Count ordered selections without repetition."),
    expandedFormula(FormulaCategory.ProbabilityCombinatorics, "Counting", "Combination formula", """{}^{n}C_{r}=\frac{n!}{r!\left(n-r\right)!}""", "n,r", "Count unordered selections without repetition."),
    expandedFormula(FormulaCategory.ProbabilityCombinatorics, "Random Variables", "Binomial probability", """P\left(X=k\right)={}^{n}C_{k}p^{k}\left(1-p\right)^{n-k}""", "X,k,n,p", "Find exactly k successes in n independent trials."),
    expandedFormula(FormulaCategory.ProbabilityCombinatorics, "Random Variables", "Poisson probability", """P\left(X=k\right)=e^{-\lambda}\frac{\lambda^{k}}{k!}""", "X,k,lambda", "Model rare event counts with mean lambda.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.ProbabilityCombinatorics, "Random Variables", "Normal density", """f\left(x\right)=\frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^{2}}""", "f,x,mu,sigma", "Evaluate the normal distribution density.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Descriptive Statistics", "Arithmetic mean", """\bar{x}=\frac{\sum_{i=1}^{n}x_{i}}{n}""", "x,i,n", "Find the average of raw data."),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Descriptive Statistics", "Standard deviation", """s=\sqrt{\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}{n-1}}""", "s,x,i,n", "Estimate typical spread from the mean.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Descriptive Statistics", "Grouped data mean", """\bar{x}=\frac{\sum f_{i}x_{i}}{\sum f_{i}}""", "x,f,i", "Find mean from a frequency table."),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Inference", "One sample t statistic", """t=\frac{\bar{x}-\mu_{0}}{\frac{s}{\sqrt{n}}}""", "t,x,mu,s,n", "Test whether a sample mean differs from a hypothesized mean.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Regression", "Simple linear regression line", """\hat{y}=a+bx""", "y,a,b,x", "Predict a response using one explanatory variable.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.StatisticsDistributions, "Continuous Distributions", "Entropy discrete", """H\left(X\right)=-\sum_{x}p\left(x\right)\log p\left(x\right)""", "H,X,x,p", "Measure uncertainty in a discrete distribution.", KnowledgeLevel.PG),
    expandedFormula(FormulaCategory.NumberTheory, "Divisibility", "HCF LCM product", """\gcd\left(a,b\right)\operatorname{lcm}\left(a,b\right)=\left|ab\right|""", "a,b", "Relate highest common factor and least common multiple."),
    expandedFormula(FormulaCategory.NumberTheory, "Divisibility", "Euclidean algorithm", """\gcd\left(a,b\right)=\gcd\left(b,a-b\left\lfloor\frac{a}{b}\right\rfloor\right)""", "a,b", "Find the greatest common divisor by repeated division."),
    expandedFormula(FormulaCategory.NumberTheory, "Modular Arithmetic", "Modular addition", """\left(a+b\right)\bmod m=\left[\left(a\bmod m\right)+\left(b\bmod m\right)\right]\bmod m""", "a,b,m", "Reduce a sum modulo m.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumberTheory, "Modular Arithmetic", "Modular multiplication", """ab\bmod m=\left[\left(a\bmod m\right)\left(b\bmod m\right)\right]\bmod m""", "a,b,m", "Reduce a product modulo m.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumericalMethods, "Root Finding", "Newton root update", """x_{n+1}=x_{n}-\frac{f\left(x_{n}\right)}{f'\left(x_{n}\right)}""", "x,n,f", "Approximate a root using tangent-line correction.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumericalMethods, "Root Finding", "Secant update", """x_{n+1}=x_{n}-f\left(x_{n}\right)\frac{x_{n}-x_{n-1}}{f\left(x_{n}\right)-f\left(x_{n-1}\right)}""", "x,n,f", "Approximate a root without computing derivatives.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumericalMethods, "Numerical Integration", "Simpson one third rule", """\int_{a}^{b}f\left(x\right)\,dx\approx\frac{h}{3}\left[y_{0}+4\sum_{\operatorname{odd}}y_{i}+2\sum_{\operatorname{even}}y_{i}+y_{n}\right]""", "f,x,a,b,h,y,i,n", "Approximate area using parabolic arcs.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumericalMethods, "Optimization", "Gradient descent update", """\mathbf{x}_{k+1}=\mathbf{x}_{k}-\alpha\nabla f\left(\mathbf{x}_{k}\right)""", "x,k,alpha,f", "Move opposite the gradient to reduce a function.", KnowledgeLevel.UG),
    expandedFormula(FormulaCategory.NumericalMethods, "Optimization", "KKT stationarity", """\nabla f\left(x\right)+\sum_{i}\lambda_{i}\nabla g_{i}\left(x\right)=0""", "f,x,i,lambda,g", "State first-order stationarity for constrained optimization.", KnowledgeLevel.PG),
)
