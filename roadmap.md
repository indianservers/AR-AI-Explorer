...
Benchmark update: `GAP_ANALYSIS.md` now contains a seven-phase implementation plan based on GeoGebra, Wolfram|Alpha, Desmos, Symbolab and Mathigon Polypad. Phase 1 starts with Desmos-quality graphing and a shared `MathObjectGraph`; later phases deepen CAS/solver exactness, GeoGebra-style geometry/tables/statistics, Polypad-style manipulatives, classroom/accessibility workflows and production ARCore spatial maths.
...
**Notebook integration update (16 July 2026):** Phase 2 of the GeoGebra/Wolfram gap plan now provides a saved-state Math Notebook foundation. Named scalar and function assignments use `:=`, exact rational values propagate through dependencies, prior scalar cells can be referenced with `#n`, redefinitions/deletions trigger atomic recomputation and diagnostics, and resolved functions can be sent to the Graph workspace as normal undoable function objects. Full shared AST objects across geometry/table/statistics, assumptions, complex/multivariate algebra, notebook file storage/import, structural maths templates and spreadsheet linkage remain follow-up work.

**Linked kernel update (16 July 2026):** the workspace now has a shared linked-object kernel that exposes CAS/algebra, graph, table, geometry, and probability views from the same definitions. Function objects carry canonical algebra, exact facts, graph samples, roots/extrema and table rows with provenance; geometry objects expose exact-style measurements; probability distributions expose summary statistics, interval probability, plot points and table rows. This is the first bridge toward GeoGebra-style synchronized views, with deeper AST editing and bidirectional manipulation still planned.

**Implementation status (16 July 2026):** the advanced graphing slice, explainable Problem Solver, probability/distribution foundation, Smart Maths Editor, interactive statistics foundation, and Maths Knowledge Intelligence foundation are implemented. Definitions can be edited, added, hidden, and deleted; the canvas renders explicit, conditional piecewise, polar, parametric, and implicit equations. The renderer-neutral layer additionally classifies inequalities, sequences, and vector fields; provides domain-aware adaptive curve sampling with discontinuity segmentation; and emits plot-ready masks, terms, vectors, ranges, and arc-length estimates. General numerical roots, extrema, intersections, tangent/normal, derivative curves, adaptive/Simpson integrals, and area-between-curves are available. A dedicated responsive Probability & Statistics workspace exposes validated Normal, Binomial, Poisson, Uniform, and Exponential models with PDF/PMF, CDF, quantiles, interval probabilities, moments, and plots. Its Statistics lab accepts editable datasets and renders tappable histograms, box plots, dot plots, ECDFs, and normal Q-Q plots. The statistics kernel supplies mean, median, multimode, quantiles, range/IQR, population and sample dispersion, standard error, coefficient of variation, absolute deviations, skewness, kurtosis, fences/outliers, confidence intervals, one-sample and Welch t tests, covariance, and Pearson/Spearman correlation. A School-to-Postgraduate study pathway now maps materials, labs, and outcomes; complete PG model-fitting workbenches for GLMs, multivariate methods, Bayesian inference, time series, and survival analysis remain planned. The Maths menu now activates searchable formulas, theorem cards, visual proof plans, dictionary terms, and a shared interactive MCQ module for Maths, Physics, Chemistry, Biology, Astro Physics, and IQ Labs; the formula library has 15 browsable categories with at least 17 LaTeX/KaTeX-style formulas in each category, and the MCQ bank has over 100 Basic/Intermediate/Advanced questions with 15-question scored quiz sessions. Analytic 2D line/circle/conic/hull APIs, analytic 3D line/plane/intersection/bounds APIs, and surface gradients/normals/tangent planes strengthen the shared maths kernel. The offline solver handles named functions/constants; linear/quadratic equations and inequalities with sign charts; arithmetic/geometric sequences and series; product, quotient, chain, general-power, trig/inverse-trig, exponential, logarithmic, root, higher and partial derivatives; common elementary antiderivatives; verified adaptive definite integrals; percentages; and descriptive statistics. Its IDE-style input performs typed syntax coloring, bracket-depth/error highlighting, cursor-aware maths insertion, diagnostics, contextual hints, capability chips, and material-style answer summaries with confidence and verification status. Remaining Phase 3 expansion includes full simplification/factorization/assumption CAS, exact units, broader natural-language parsing, geometry proofs, UI rendering/editing for inequality/sequence/vector-field definitions, multiple named parameters with persisted ranges, complex plots, matrices/spreadsheet cells, regression/residual workbenches, categorical tests, ANOVA, non-parametric tests, simulation/resampling, more distributions, PG model fitting, ODE fields, interval arithmetic, and a full structural 2D maths keyboard.
...
**Implementation status (16 July 2026):** the ARCore spatial production layer is implemented as an optional Android feature. It includes a shared OpenGL ES renderer used by studio 3D, surface graphs and AR; a live camera compositor/frame loop; real plane/depth/point hit tests; native anchors and relocalization state; capability-gated depth, environment lighting, safe placement bounds, uncertainty-labelled educational measurements, adaptive thermal quality, persisted mathematical poses, and a full non-AR simulator. Six spatial maths lessons and a detached future classroom-session envelope are included. Cloud/shared-anchor transport remains intentionally future work. Supported-device drift, thermal, occlusion and privacy validation remain physical release QA under AR_PRIVACY_SAFETY_CHECKLIST.md.
